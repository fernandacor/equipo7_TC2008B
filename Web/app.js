...
app.put('/api/personajes', async (request, response) => {
    ...
    try {
        ...
        const [results, fields] = await connection.query('update personajes set idArma = ?, energia = ?, xp = ?, velocidadMov = ?, velocidadDis = ?, vida = ?, resistencia = ?, recuperacionEn = ?, enemiesKilled = ?, damageDealt = ?, coinsTaken = ? WHERE idPartida = ?', [request.body['idArma'], request.body['energia'], request.body['xp'], request.body['velocidadMov'], request.body['velocidadDis'], request.body['vida'], request.body['resistencia'], request.body['recuperacionEn'], request.body['enemiesKilled'], request.body['damageDealt'], request.body['coinsTaken'], request.body['idPartida']])
